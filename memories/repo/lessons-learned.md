...
- **Interpreting Prompt Files as Actions:** When a user provides a prompt file (like `merge.md`), they intend for you to execute the prompt's instructions, NOT literally append the prompt's content into another file.
- **Executing Multi-Phase Routines:** When instructed to execute a multi-phase prompt/routine, do not pause and ask for permission to proceed to the next phase unless explicitly instructed to do so. Complete all phases of the routine autonomously to avoid halting the workflow.
